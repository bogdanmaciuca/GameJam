...
void DrawGUI(sf::RenderWindow& window, int mana) {
	int width = 200, height = 40;
	sf::RectangleShape outter({ (float)width + 20, (float)height + 20 }), inner({ (float)width, (float)height }), active({ (float)width * mana / MAX_MANA, (float)height });
